...
# Author: Govinda
## Tile coding  probelm
## High  space  where it can touch  line [LINK_LENGTH_1, LINK_LENGTH_2, LINK_MASS_1,LINK_MASS_2, self.MAX_VEL_1, self.MAX_VEL_2]
## low space is minus  of high -high [-LINK_LENGTH_1, -LINK_LENGTH_2, -LINK_MASS_1,-LINK_MASS_2, -self.MAX_VEL_1, -self.MAX_VEL_2]
#################################
import sys
import gym
import numpy as np
...
def lets_add_arguments():
    ...
    parser.add_argument("--display-sample-tile", help="Display Grid", action="store_true")
    parser.add_argument("--sample-q-table", help="Train Q model with 10 bins", action="store_true")
    parser.add_argument("--train-agent", help="Train Q model with 10 bins", action="store_true")
    parser.add_argument("--Qmodel-run", help="Run an exampke from saved model", action="store_true")
    return parser

def lets_get_arguments(parser):
    args = parser.parse_args()
    if args.display_sample_tile:
        visulize_tilings_with_mappings()
...
def visulize_tilings_with_mappings():
    samples = [(-1.2, -5.1),
               (-0.75, 3.25),
               (-0.5, 0.0),
               (0.25, -1.9),
...
class TiledQTable:
    ...
    def __init__(self, low, high, tiling_specs, action_size,qtable_data,savedtable='no'):
...
def sample_qtable():
    ...
    tiling_specs = [(bins, -offset_pos),
                    ...
                    (bins, offset_pos)]
    tq = TiledQTable(low, high, tiling_specs, env.action_space.n,qtable_data)
    samples = [[-0.8,-0.7,-0.7,-0.7,-12,-21],[-0.4,-0.3,-0.2,-0.1,-10,-15]]
    s1 = 0;
    s2 = 1;
    a = 0;
    q = 1.0
...
def train_the_agent():
    ...
    tq = TiledQTable(env.observation_space.low,
                     ...
                     tiling_specs,
                     env.action_space.n,qtable_data)
    agent = QLearningAgent(env, tq)
    scores = run(agent, env)
    rolling_mean = plot_scores(scores)
    np.save(f"acrobat-qtable.npy", tq.q_tables)
    #let_make_test_run(tq)
...
def let_make_test_run():
    ...
    print(len(bins))
    offset_pos = (env.observation_space.high - env.observation_space.low) / (3 * n_bins)
    qtable_data1 = np.load(f"acrobat-qtable.npy",allow_pickle=True)
    qtable_data = 'yes'
    tiling_specs = [(bins, -offset_pos),
                    (bins, tuple([0.0] * env.observation_space.shape[0])),
                    (bins, offset_pos)]
    tq = TiledQTable(low, high, tiling_specs, env.action_space.n,qtable_data, qtable_data1)
    learnet_agent = QLearningAgent(env, tq)
    state = env.reset()
    score = 0
    count = 0
    for t in range(400):
